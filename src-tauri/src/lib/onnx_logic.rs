...
/// 自动加载模型和分词器，无需前端传递路径。
#[tauri::command]
pub async fn generate_bundled_embedding(
  texts: Vec<String>,
  app: AppHandle,
) -> Result<Vec<Vec<f32>>, String> {
  tauri::async_runtime::spawn_blocking(move || {
    ...
    let session = guard.as_mut().ok_or("ONNX session not available")?;
    ...
    let mask_flat: Vec<i64> = encodings
      ...
      .collect();
    let token_type_flat: Vec<i64> = vec![0i64; batch * seq_len];

    let mask_array =
      Array2::from_shape_vec((batch, seq_len), mask_flat.clone())
        .map_err(|e| format!("Failed to create mask array: {}", e))?;

    let ids_tensor =
      Tensor::<i64>::from_array(([batch, seq_len], ids_flat))
        .map_err(|e| format!("Failed to create ids tensor: {}", e))?;
    let mask_tensor =
      Tensor::<i64>::from_array(([batch, seq_len], mask_flat))
        .map_err(|e| format!("Failed to create mask tensor: {}", e))?;
    let token_type_tensor =
      Tensor::<i64>::from_array(([batch, seq_len], token_type_flat))
        .map_err(|e| format!("Failed to create token_type tensor: {}", e))?;
    ...
    let shape = hidden_state.shape();
    if shape.len() != 3 {
      return Err(format!("Unexpected output dimensions: {:?}", shape));
    }
    let hidden_size = shape[2];

    let data = hidden_state.as_slice().ok_or("Cannot get output data")?;
